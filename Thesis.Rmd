...
std_dataM$meanict <- (std_dataM$G_Q05an + std_dataM$G_Q05cn + std_dataM$G_Q05dn + std_dataM$G_Q05en + std_dataM$G_Q05gn + std_dataM$G_Q06n + std_dataM$G_Q07n + std_dataM$G_Q08n)/8

# Creation  of a more general index of ICT with only 3  variables 

std_dataM$meanictBasic <- (std_dataM$G_Q04n + std_dataM$G_Q04_Tn + std_dataM$COMPUTEREXPERIENCE)/3

# Index of Education status

std_dataM$meanEducation <- (std_dataM$B_Q01an + std_dataM$B_Q01a_Tn + std_dataM$B_Q02an + std_dataM$B_Q01bn)/4

# Index of Work status

std_dataM$meanWorkS <- (std_dataM$C_Q01an + std_dataM$D_Q03n + std_dataM$D_Q14n + std_dataM$D_Q18a_T)/4
...
std_dataM$meanSoftS <- (std_dataM$F_Q01bn + std_dataM$F_Q02bn + std_dataM$F_Q02cn + std_dataM$F_Q02dn + std_dataM$F_Q02en + std_dataM$F_Q03bn + std_dataM$F_Q04an + std_dataM$F_Q05an + std_dataM$F_Q05bn)/9

# Index of Numeracy skills 

std_dataM$meanNumeracyS <- (std_dataM$G_Q03bn + std_dataM$G_Q03cn + std_dataM$G_Q03dn + std_dataM$G_Q03fn + std_dataM$G_Q03gn + std_dataM$G_Q03hn)/6

# Index of Literacy skills 

std_dataM$meanLiteracyS <- (std_dataM$G_Q01an + std_dataM$G_Q02an + std_dataM$G_Q02bn + std_dataM$G_Q02cn + std_dataM$G_Q02dn)/5
...
#Percentage of missings by type of skills 
missing.values <- ICTSKILLS %>%
  gather(key = "key", value = "val") %>%
...
percentage.plot2 <- missing.values1 %>%
      ggplot() +
        geom_bar(aes(x = reorder(key, desc(pct)), 
                     y = pct, fill=isna), 
                  stat = 'identity', alpha=0.8) +
...
GRAPH1 <-subMexico %>% 
  select("GENDER_Rn", "AGE_R", "G_Q05an", "G_Q05cn","G_Q05dn", "G_Q05en",  "G_Q05gn",  "G_Q06n" ,  "G_Q07n", "G_Q08n")
...
tapply(subMexico,
        list(subMexico$GENDER_Rn,subMexico$AGE_R), mean)

StackedBar = ggplot(subMexico, aes(G_Q05an, AGE_R), fill = "GENDER_Rn", inherit.aes = FALSE) + 
  geom_bar(stat="summary", fun.y="mean") + 
  scale_fill_brewer(palette = "Set2") + ggtitle("Humor Chart")
StackedBar


GroupedBar = ggplot(subMexico, aes(GENDER_Rn, GQ05cn, fill = AGE_R),inherit.aes = FALSE) + geom_bar(stat="summary", fun.y="mean", position = "dodge") + 
  scale_fill_brewer(palette = "Set2") + ggtitle("Humor Chart")
GroupedBar
