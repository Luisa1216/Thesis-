...
#Mexico <- read.csv("/Users/lu/Dropbox/Hertie/3/Thesis/Thesis-/prgmexp1.csv")
Mexico <- read.csv("/Users/mac/Dropbox/Thesis/Thesis-/prgmexp1.csv")
...
#C_Q01a
newMexico1$C_Q01an[newMexico1$C_Q01a == "N"] = NA
...
newMexico1$G_Q04_Tn[newMexico1$G_Q04_T == "N"] = NA
newMexico1$G_Q04_Tn[newMexico1$G_Q04_T == "1"] = 0
newMexico1$G_Q04_Tn[newMexico1$G_Q04_T == "2"] = 1
...
std_dataM= as.data.frame(apply(subMexico,2,standardize))

# Creation of the first index of ICT skills for work 

subMexico$meanict <- (std_dataM$G_Q05an + std_dataM$G_Q05cn + std_dataM$G_Q05dn + std_dataM$G_Q05en + std_dataM$G_Q05gn + std_dataM$G_Q06n + std_dataM$G_Q07n + std_dataM$G_Q08n)/8

# Creation  of a more general index of ICT with only 3  variables 

subMexico$meanictBasic <- (std_dataM$G_Q04n + std_dataM$G_Q04_Tn + std_dataM$COMPUTEREXPERIENCE)/3

# Index of Education status

subMexico$meanEducation <- (std_dataM$B_Q01an + std_dataM$B_Q01a_Tn + std_dataM$B_Q02an + std_dataM$B_Q01bn)/4

# Index of Work status

subMexico$meanWorkS <- (std_dataM$C_Q01an + std_dataM$D_Q03n + std_dataM$D_Q14n + std_dataM$D_Q18a_T)/4
...
subMexico$meanSoftS <- (std_dataM$F_Q01bn + std_dataM$F_Q02bn + std_dataM$F_Q02cn + std_dataM$F_Q02dn + std_dataM$F_Q02en + std_dataM$F_Q03bn + std_dataM$F_Q04an + std_dataM$F_Q05an + std_dataM$F_Q05bn)/9

# Index of Numeracy skills 

subMexico$meanNumeracyS <- (std_dataM$G_Q03bn + std_dataM$G_Q03cn + std_dataM$G_Q03dn + std_dataM$G_Q03fn + std_dataM$G_Q03gn + std_dataM$G_Q03hn)/6

# Index of Literacy skills 

subMexico$meanLiteracyS <- (std_dataM$G_Q01an + std_dataM$G_Q02an + std_dataM$G_Q02bn + std_dataM$G_Q02cn + std_dataM$G_Q02dn)/5
...
std_dataM$ICTWork1pca<- prcomp(std_dataM[,c("G_Q05an", "G_Q05cn", "G_Q05dn" , "G_Q05en" , "G_Q05gn" , "G_Q05hn")], na.action=na.omit, center = TRUE,scale. = TRUE)
...
subMexico$ICTWork2.pca <- prcomp(subMexico[,c("G_Q05an", "G_Q05cn", "G_Q05en" , "G_Q05hn")], na.action=na.omit, center = TRUE,scale. = TRUE)
...
std_dataM$LiteracyworkPAC  <- prcomp(newMexico1ICTwork1[,c("G_Q01an" , "G_Q02an" , "G_Q02bn" , "G_Q02cn" , "G_Q02dn")], na.action=na.omit, center = TRUE,scale. = TRUE)
...
variablemissingsbygender<-GRAPH1%>%
  group_by(GENDER_Rn) %>%
  miss_var_summary()


missing.values7 <- GRAPH1%>%
...
missing.values7 %>%
  ggplot() +
    geom_bar(aes(x=key, y=num.missing, fill=num.missing), stat = 'identity') +
    labs(x='Variables', y="NAs", 
         title='Number of missing values') +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))

YAY<-variablemissingsbygender%>%
  ggplot() +
...
missing.values7 %>%
  ggplot() +
    ...
    labs(x='Variables', y="Number of NAs", 
         title='Number of missing values') +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))



ggplot(GRAPH1,geom_bar(aes(x=AGE_R, y=num.missing)), fill = "GENDER_Rn") 


GRAPH1 %>%
ggplot()+geom_bar(aes(x=AGE_R, y=num.missing), fill = "GENDER_Rn") 

# Variables por genero y edad 
auxilio<-new_DF1%>%
ggplot()+geom_bar(aes(x=AGE_R, y=G_Q05an, fill= GENDER_Rn),stat= 'identity')
auxilio

auxilio2<-new_DF%>%
ggplot()+geom_bar(aes(x=AGE_R, y=meanict, fill= GENDER_Rn),stat= 'identity')
...
levels <- (missing.values1  %>% filter(isna == T) %>%     
...
#Percentage of missings soft skills 
missing.values2 <- softskills %>%
  gather(key = "key", value = "val") %>%
  mutate(isna = is.na(val)) %>%
  group_by(key) %>%
  mutate(total = n()) %>%
...
percentage.plot2 <- missing.values2 %>%
      ggplot() +
        ...
        scale_fill_manual(name = "", 
                          ...
                          labels = c("Present", "Missing")) +
        coord_flip() +
        labs(title = "Percentage of missing values soft skills", 
             x = 'Variable', y = "% of missing values")
percentage.plot2

#Percentage of missings Literacy Skills

missing.values3 <- Literacyskills %>%
  gather(key = "key", value = "val") %>%
  mutate(isna = is.na(val)) %>%
  group_by(key) %>%
  mutate(total = n()) %>%
...
percentage.plot3 <- missing.values3 %>%
      ggplot() +
        ...
        scale_fill_manual(name = "", 
                          ...
                          labels = c("Present", "Missing")) +
        coord_flip() +
        labs(title = "Percentage of missing values Literacy skills", 
             x = 'Variable', y = "% of missing values")
percentage.plot3
...
new_DF1 <- GRAPH1[rowSums(is.na(GRAPH1)) > 0,]
new_DF <- subMexico2[rowSums(is.na(subMexico2)) > 0,]

#AGE in ranges 
# Creating vectors 
AGE_R <- c(16:65) 
# Creating a factor corresponding to age with labels 
wfact = cut(subMexico$AGE_R, 3, labels = c('Young', 'Medium', 'Aged')) 
table(wfact) 

#basic info AGE 
with(subMexico, summary(AGE_R))
